  ...
  def draw(self, g):
    ...
    for grid_y in range(0, self.grid_height):
      for grid_x in range(0, self.grid_width):
        ...
        g.fill_rect(x, y, self.button_width, self.button_height, color=back_color)
        if selected:
          t = self.click_timer / 1.0
          g.fill_rect(x, y, int(self.button_width * t), self.button_height, color=color.WHITE)
        g.draw_rect(x, y, self.button_width, self.button_height, color=border_color)
        c = self.grid[grid_y][grid_x]
        if isinstance(c, tuple):
          c = c[0]
        g.draw_text(cx, cy, text=c, color=color.BLACK, align=Align.Centered)
