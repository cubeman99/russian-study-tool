...
class ReadTextState(State):
  ...
  def begin(self):
    ...
    self.paragraphs = [(AccentedText(p), list(split_words(AccentedText(p).text))) for p in self.app.example_database.stories[2].chapters[0].paragraphs]
  ...
  def draw(self, g):
    ...
    for paragraph_text, paragraph_words in self.paragraphs:
      ...
      for name, start_index in paragraph_words:
        ...
        if visible:
          if len(cards) > 0:
            back_color = math.lerp(color.RED, color.GREEN, t=cards[0].get_history_score())
